const CACHE="huspass-shell-v3";
const SHELL=["/manifest.webmanifest","/huspass-icon.svg","/huspass-maskable.svg"];
...
self.addEventListener("fetch",event=>{const req=event.request;if(req.method!=="GET")return;const url=new URL(req.url);if(url.origin!==self.location.origin)return;const isPublicAsset=url.pathname==="/manifest.webmanifest"||url.pathname==="/huspass-icon.svg"||url.pathname==="/huspass-maskable.svg";if(!isPublicAsset)return;event.respondWith(caches.match(req).then(hit=>hit||fetch(req).then(res=>{if(res.ok){const copy=res.clone();caches.open(CACHE).then(cache=>cache.put(req,copy))}return res})))});
